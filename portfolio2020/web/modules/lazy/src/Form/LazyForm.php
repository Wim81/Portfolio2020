...
class LazyForm extends ConfigFormBase {
  ...
  protected function getEnabledFiltersAndFields(array $fields) {
      ...
      if ($image_fields[$tag]) {
        ...
        if (($entity_type !== NULL) && ($entity_bundle !== NULL)) {
          ...
          $links['field'][$tag] = [
            ...
            'title' => $entity_bundle . '.' . $field_name . " ($view_mode)",
          ];
        }
      }
    }
    ...
    $this->messenger()->addMessage($this->t("The <strong>fields</strong> have lazy-loading enabled: ${field_links_result}"), $field_message_type);
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $plugins = array_keys(\Drupal::service('lazy')->getPlugins());
